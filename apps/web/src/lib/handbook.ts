...
export type HandbookDoc = {
  /** Path under root with forward slashes, e.g. `folder/note.md` */
  relPath: string;
  /** URL segments without `.md`, e.g. `['folder', 'note']` */
  slug: string[];
  title: string;
};

/** Nested folders + loose files at handbook root (for sidebar). */
export type HandbookNavFolder = {
  name: string;
  /** Segments from handbook root through this folder (e.g. `['Specs', 'API']`). */
  pathSegments: string[];
...
/** Turn a flat doc list into a folder tree matching on-disk structure. */
